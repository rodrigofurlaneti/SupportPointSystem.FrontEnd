...
export default function AdminCustomerList() {
  ...
  const handleDelete = async (id: string, name: string) => {
    if (!window.confirm(`Deseja remover permanentemente: ${name}?`)) return;
    try {
            ...
            {filtered.map((customer) => (
              ...
              >
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h3 className="font-black text-base uppercase tracking-tight">{customer.companyName}</h3>
                    <p className="text-slate-400 text-xs mt-0.5">{customer.cnpj}</p>
                    {customer.address && (
                      <p className="text-slate-500 text-xs flex items-center gap-1 mt-0.5">
                        <MapPin size={10} /> {customer.address.city} - {customer.address.state}
                      </p>
                    )}
                  </div>
                </div>
