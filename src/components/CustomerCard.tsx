...
export function CustomerCard({ customer, onSelect, onCheckIn, onCheckOut }: CustomerCardProps) {
  return (
    <div className="bg-check-card p-5 rounded-[2.5rem] border border-white/5 shadow-xl transition-all active:scale-[0.98]">
      <div
        className="flex gap-4 items-start cursor-pointer mb-5"
      ...
      >
        ...
        <div className="flex-1">
          <h4 className="font-black text-base text-white uppercase leading-tight tracking-tight">
      ...
      <div className="grid grid-cols-2 gap-3">
        <button
          onClick={() => onCheckIn(customer)}
          className="bg-check-green hover:bg-lime-500 text-check-blue py-4 rounded-2xl text-[10px] font-black uppercase flex items-center justify-center gap-2 shadow-lg shadow-check-green/10 transition-all"
        >
          <CheckCircle2 size={16} strokeWidth={3} /> Check-in
        </button>
        <button
          onClick={() => onCheckOut(customer)}
          className="bg-red-500/10 hover:bg-red-500 text-red-500 hover:text-white border border-red-500/20 py-4 rounded-2xl text-[10px] font-black uppercase flex items-center justify-center gap-2 transition-all"
        >
